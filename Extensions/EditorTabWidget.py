...
from Extensions.AsEditor import AsEditor
...
class EditorTabWidget(QtWidgets.QTabWidget):
    ...
    def __init__(
        ...
        self.backupTimer.setInterval(60000)#60000
        ...
        self.newFileMenu.addAction(self.newAsFileAct)
    ...
    def createActions(self):
        ...
        self.convertToAsAct = \
            QtWidgets.QAction(
                QtGui.QIcon(os.path.join("Resources", "images", "convert")),
                "Convert to As", self, statusTip="Convert to As",
                triggered=self.convertToAs)
        ...
        self.newAsFileAct = QtWidgets.QAction(
            QtGui.QIcon(os.path.join("Resources", "images", "new")),
            "AS", self,
            statusTip="Create a new as file",
            triggered=self._newAsFile)
    ...
    def splitVertical(self):
        ...
        splitter.splitter.setOrientation(2)
        splitter.splitter.widget(1).show()

    def splitHorizontal(self):
        splitter = self.currentWidget().widget(0)
        splitter.splitter.setOrientation(1)
        splitter.splitter.widget(1).show()

    def removeSplit(self):
        splitter = self.currentWidget().widget(0)
        splitter.splitter.widget(1).hide()
    ...
    def restoreSession(self):
        ...
        if os.path.exists(loadPath) is not True:
            self.clearBackups()
            loadPath = self.projectPathDict["session"]
    ...
    def convertToAs(self):
        path = self.getEditorData("filePath")
        parent = executable_path()
        if self.getEditorData("fileType") == "as":
            execPath = os.path.abspath(os.path.join(parent,"ascompiler/kawasaki.exe"))
            args = "{} -s={}".format(execPath,path)
            process = subprocess.Popen(args)
            print('the status code is:', process.returncode)
            # with open('log', 'w') as object:
            #     object.write(process.stderr.read())
    ...
    def saveAs(self, index=None, copyOnly=False):
        ...
        fileName,options = QtWidgets.QFileDialog.getSaveFileName(self,
                                                     "Save As", os.path.join(self.useData.getLastOpenedDir(),
                                                                             self.getTabName()), filter) #
        if fileName:
            #fopenDir = os.path.split(fileName)[0]
    ...
    def openFile(self):
        fileName,options = QtWidgets.QFileDialog.getOpenFileName(self,
                                                     "Select File", self.useData.getLastOpenedDir(
                                                     ),
                                                     "All Files (*);;Console (*.py);;No Console (*.pyw);;As (*.as);;")
        if fileName:
            self.useData.saveLastOpenedDir(os.path.split(fileName)[0])
    ...
    def _newAsFile(self):
        self.newEditor(fileName="Untitled.as")
    ...
    def newEditor(self, index=None, fileName="Untitled.py",
        ...
        if pyFile:
            ...
            if extension in [".as", ".pg"]:
                extension = "as"
            DATA["fileType"] = extension
            #TextEditor
            editor = AsEditor(self.useData, self.refactor, self.colorScheme,
                                DATA, self)
            editor2 = AsEditor(self.useData, self.refactor, self.colorScheme,
                                 DATA, self)
            # editor = AsEditor(self.useData, DATA, self.colorScheme, self,
            #                     encoding)
            # editor2 = AsEditor(self.useData, DATA, self.colorScheme, self,
            #                      encoding)
