...
from Extensions.Outline.As import asclbr
...
class AsOutlineThread(QtCore.QThread):
    updateNavigator = QtCore.pyqtSignal(dict)
    def run(self):
        outlineDict =asclbr._readmodule(self.source)
        self.updateNavigator.emit(outlineDict)
...
class Outline(QtWidgets.QTreeWidget):
    ...
    def __init__(self, useData, editorTabWidget, parent=None):
        ...
        self.asOutlineThread = AsOutlineThread()
        ...
        self.asOutlineThread.updateNavigator.connect(self.updateAsOutline)
    ...
    def startOutline(self):
        ...
        fileType = self.editorTabWidget.getEditorData("fileType")
        if fileType == "python":
            self.pythonOutlineThread.startNavigator(
                self.editorTabWidget.getSource())
        elif fileType == "as":

            self.asOutlineThread.startNavigator(
                self.editorTabWidget.getSource())
        else:
            self.clear()

    def updateAsOutline(self,outlineDict):
        self.clear()
