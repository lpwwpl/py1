...
from environment_pick_place_stereo import *
import pdb
import threading
import kdata_lx
...
class KCore(QtCore.QThread):
    ...
    def act(self):  # pylint: disable=unused-argument
        ...
        _, hmap, obj_mask = self.env.get_true_image()

        # Unpack next goal step.
        objs, matches, targs, replace, rotations, _, _, _ = self.env.goals[0]
        ...
        pick_pos = utils.pix_to_xyz(pick_pix, hmap,
                                    self.env.bounds, self.env.pix_size)
    ...
    def text_read(self):
        ...
        if kdata_lx.isReal:
            ...
            Cam2Base_rm = [[   0.048402,     0.94935,     0.31047 ,     0.00],
                             [   0.032417 ,    0.30918,    -0.95045 ,    -0.155],
                             [    -0.9983,    0.056068,    -0.01581 ,   30],
                             [          0 ,          0 ,          0  ,         1]]
        return Cam2Base_rm
...
class Kawasaki_lx(QWidget):
    ...
    def initUI(self):
        ...
        self.env = Environment_Pick_Place_Stereo()
        ...
        # self.btnDepth.setFixedSize(160,50)
        # self.btnDepth.clicked.connect(self.calDepth)
    ...
    def virt_capture(self):
        if kdata_lx.isVirt and self.env:
            ...
            l_filepath = "picsStereo/lpw_l.color.png"
            r_filepath = "picsStereo/lpw_r.color.png"
            deptfilepath = "picsStereo/lpw.depth.png"
            cv.imwrite(l_filepath, kdata_lx.color_l_streaming)
            cv.imwrite(r_filepath, kdata_lx.color_r_streaming)
            # depth_mat = np.zeros((480,640),np.float32)
            #
            #
            # for y in range(480):
