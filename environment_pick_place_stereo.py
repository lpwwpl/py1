...
ASSETS_PATH = './assets/'
class Environment_Pick_Place_Stereo(gym.Env):
    def __init__(self):
        ...
        self.update_l_camera_image_thread = None
        self.update_r_camera_image_thread = None
        ...
        self.pix_size = 0.003125
        self.bounds = np.array([[-0.2, 0.2], [-0.2, 0.2], [0, 0.3]])
    ...
    def reset(self):
        ...
        zone_size = self.get_random_size(0.05, 0.15, 0.05, 0.15, 0.05, 0.15)
        zone_pose = [[0,-0.3,0.01],[0,0,0,1]]
        container_template = 'container/container-template.urdf'
        half = np.float32(zone_size) / 2
        replace = {'DIM': zone_size, 'HALF': half}

        container_urdf = self.fill_template(container_template, replace)
        containerid=self.add_object(container_urdf, zone_pose, 'fixed')
        print(containerid)
    ...
    def open(self):
        try:
            ...
            self.oracle_cams = cameras.Oracle.CONFIG
            self.camera_l = Camera(self.camera_intrinsic)
            self.camera_r = Camera(self.camera_intrinsic)
    ...
    def movej(self, targj, speed=0.001, timeout=50):
    ...
    def t2c_l(self, end_pos, end_orn):
        relative_offset = [0.05, 0.005, 0.05]
    ...
    def t2c_r(self, end_pos, end_orn):
        relative_offset = [0.05, -0.005, 0.05]
        end_orn = R.from_quat(end_orn).as_matrix()
        end_x_axis, end_y_axis, end_z_axis = end_orn.T
        wcT = np.eye(4)  # w: world, c: camera, ^w_c T
    ...
    def _bind_l_camera_to_end(self, end_pos, end_orn):
        """设置相机坐标系与末端坐标系的相对位置
        ...
        """
        relative_offset = [0.05, 0.005, 0.05]  # 相机原点相对于末端执行器局部坐标系的偏移量
        end_orn = R.from_quat(end_orn).as_matrix()
        end_x_axis, end_y_axis, end_z_axis = end_orn.T
    ...
    def _bind_r_camera_to_end(self, end_pos, end_orn):
        """设置相机坐标系与末端坐标系的相对位置
    ...
    #     self.l_frame = self.camera_l.render(wcT)
    #
    #     rpy = rm2rpy(wcT[:3,])
    #
    ...
    def update_l_camera_image(self):
        try:
            while True:
                if self.stope_:
                    break
                ...
                if not p.isConnected():
                    break
                wcT = self._bind_l_camera_to_end(end_pos, end_orn)
                ...
                t2c_l = self.t2c_l(end_pos, end_orn)
                self.l_frame.t2c = t2c_l

                ###################################
                rpy = rm2rpy(wcT[:3, ])
                self.oracle_cams[1]['position'] = (end_pos[0] + 0.05, end_pos[1]+0.005, end_pos[2] + 0.05)
                self.oracle_cams[1]['rotation'] = p.getQuaternionFromEuler((rpy[3], rpy[4], rpy[5]))

                # imgs = self._get_obs()
                assert isinstance(self.l_frame, Frame)
                ...
                color, depth, segm = self.render_camera(self.oracle_cams[1])
                kdata_lx.color_l_streaming = color
                # kdata_lx.color_l_streaming = depth
    ...
    def update_r_camera_image(self):
        try:
            while True:
                if self.stope_:
                    break
                ...
                if not p.isConnected():
                    break
                wcT = self._bind_r_camera_to_end(end_pos, end_orn)

                # cwT = np.linalg.inv(wcT)
                if not p.isConnected():
                    break
                # self.frame = self.camera.render(cwT)
                self.r_frame = self.camera_r.render(wcT)


                t2c_r = self.t2c_r(end_pos, end_orn)
                self.r_frame.t2c = t2c_r

                ###################################
                rpy = rm2rpy(wcT[:3, ])
                self.oracle_cams[0]['position'] = (end_pos[0] + 0.05, end_pos[1]-0.005, end_pos[2] + 0.05)
                self.oracle_cams[0]['rotation'] = p.getQuaternionFromEuler((rpy[3], rpy[4], rpy[5]))

                # imgs = self._get_obs()
                assert isinstance(self.r_frame, Frame)
                ...
                color, depth, segm = self.render_camera(self.oracle_cams[0])
                kdata_lx.color_r_streaming = color
                # kdata_lx.color_r_streaming = depth
                # kdata_lx.depth_streaming =
                # cv2.imshow("image", bgr)
                # key = cv2.waitKey(1)
                time.sleep(0.02)
    ...
    def stop_manual_control(self):
        # self.client.close()
        self.stope_ = True
        if self.update_l_camera_image_thread and self.update_l_camera_image_thread.is_alive():
            self.update_l_camera_image_thread.join()
        if self.update_r_camera_image_thread and self.update_r_camera_image_thread.is_alive():
            self.update_r_camera_image_thread.join()
    ...
    def start_manual_control(self):
        ...
        if not self.update_l_camera_image_thread or not self.update_l_camera_image_thread.is_alive():
            self.update_l_camera_image_thread = threading.Thread(
                target=self.update_l_camera_image)
            # self.update_camera_image_thread.setDaemon(True)
            self.update_l_camera_image_thread.start()

        if not self.update_r_camera_image_thread or not self.update_r_camera_image_thread.is_alive():
            self.update_r_camera_image_thread = threading.Thread(
                target=self.update_r_camera_image)
            # self.update_camera_image_thread.setDaemon(True)
            self.update_r_camera_image_thread.start()
