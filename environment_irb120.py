...
UR5_URDF_PATH = 'abb/irb120/irb120.urdf'
...
class Environment(gym.Env):
    def __init__(self):
        ...
        self.urdf_file = "./assets/abb/irb120/irb120.urdf"
        self.stope_ = False
        ...
        self.ee_tip = 8  # Link ID of suction cup.
        # self.ee_tool_fixed_joint = 7 # Link ID of suction cup.
        self.pick_place = PickPlace()
        # self.agent_cams = cameras.RealSenseD435.CONFIG
        self.pix_size = 0.003125
        self.homej = np.array([-1, -0.5, 0.5, 0, 1.5707963, 0])
        # np.array([-1, -0.5, 0.5, -0.5, -0.5, 0]) * np.pi
        # self.open()
    ...
    def reset(self):
        ...
        pose_chess = [[0.60, 0, 0.01], [0, 0, 0,1]]
    ...
    def open(self):
        try:
            ...
            self.base_axes = DebugAxes()  # 机械臂末端的局部坐标系
    ...
    def movej(self, targj, speed=0.01, timeout=50):
        ...
        while (time.time() - t0) < timeout:
            ...
            if all(np.abs(diffj) < 1e-2):
                print("pose by movep,", self.get_current_tcp())
                print("joints by movep,", currj)
    ...
    def movep_6dof(self, pose, acc=0.01, speed=0.01,active=False):
        ...
        rm = self.Rypr(pose[3],pose[4],pose[5])
        HM = np.zeros([4, 4], dtype=np.float64)
        HM[0:3, 0:3] = rm
        p_array = np.asarray(pose)
        HM[0:3, 3] = p_array[:3].reshape(-1)
        HM[3, 0:4] = [0.0, 0.0, 0.0, 1.0]
        #
        qua = self.rotateToQuaternion(rm)

        # qua = p.getQuaternionFromEuler(pose[3:6])
        # qua =(4.540828239175913e-18, 0.7071067811848163, -5.880363679206258e-18, 0.7071067811882787)
        # qua = (1,0,0,0)
        pose = (xyz,qua)
    ...
    def _bind_camera_to_end(self, end_pos, end_orn):
        ...
        wcT = np.eye(4)  # w: world, c: camera, ^w_c T
        # wcT[:3, 0] = -end_z_axis  # camera x axis
        # wcT[:3, 1] = end_y_axis  # camera y axis
    ...
    def get_base_state(self):
        ...
        """
        end_state = p.getLinkState(self.ur5, 2)
        end_pos = end_state[0]
        end_orn = end_state[1]
    ...
    def update_debug_axes(self):
        try:
            while True:
                ...
                # base_pos,base_orn=self.get_base_state()
                #
                # self.base_axes.update(base_pos,base_orn)
                if not p.isConnected():
                    break
    ...
    def start_manual_control(self):
        ...
        # rpy2qt([0.500, -0.000, 0.3,0,0,0])
        # pose = [(0.599999999999966, 0.2, 0.3300000000005872), (1, 0.0, 0.0, 0)]
        # self.movep(pose)
        # tcp = self.get_current_tcp()
        # print("tcp,",tcp)
        # # qua = p.getQuaternionFromEuler([tcp[3],tcp[4],tcp[5]])
        # # tcp[3]= 0.47943
        # # tcp[4]=0.87758
        # # tcp[5]=-6.4243e-09
        # rm = self.Rypr(tcp[3],tcp[4],tcp[5])
        # HM = np.zeros([4, 4], dtype=np.float64)
        # HM[0:3, 0:3] = rm
        # HM[0:3, 3] = tcp[:3].reshape(-1)
        # HM[3, 0:4] = [0.0, 0.0, 0.0, 1.0]
        # #
        #
        # qua = self.rotateToQuaternion(rm)
        ...
        # # print("rpy,", rpy)
        # #
        # # qua = p.getQuaternionFromEuler([rpy[5],rpy[4],rpy[3]])
        # # qua=[qua[1],qua[2],qua[3],qua[0]]
        # pose = [tcp[:3],qua]
        # print("homej,",self.homej)
        # self.movep(pose)
        tarj = [-0.35417,     0.50821,   -0.45585  ,   0.25617  ,    0.9969  ,         0]
        self.movej(tarj,speed=0.01)
...
if __name__ == "__main__":
    ...
    # 4.540828239175913e-18, 0.7071067811848163, -5.880363679206258e-18, 0.7071067811882787
    params=[1, 0, 0, 0]
    rpy = quaternion_to_euler(params)
    print(rpy)

    qt = rpy2qt([0,0,0,3.1415926/2, 0, 0])
    print(qt)
    # env.reset()
