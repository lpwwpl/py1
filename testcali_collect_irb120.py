...
from environment_irb120 import Environment
from Kawasaki_Control import Kawasaki_cntl_diag
from Kawasaki_Control import Kawasaki_cntl_virt_diag
...
class Kawasaki_cali_collect(QWidget):
    ...
    workspace_limits = [[0.50, 0.56], [-0.03, 0.03], [0.54002, 0.58573]]
    ...
    def __init__(self, parent=None):
        ...
        self.control_Virt_dialog = Kawasaki_cntl_virt_diag()
    ...
    def capture(self):
        ...
        if kdata_lx.isVirt:
            ...
            colorFileName = "{}\{}".format(os.path.abspath(self.save_dir), "Color_{:0>2d}.png".format(self.capture_num))
            depthFileName = "{}\{}".format(os.path.abspath(self.save_dir), "Depth_{:0>2d}.png".format(self.capture_num))
            colorFileName = increment_path(colorFileName)
            depthFileName = increment_path(depthFileName)
            color_img = cv.cvtColor(kdata_lx.colorImg, cv2.COLOR_RGB2BGR)
            cv.imwrite(colorFileName.as_posix(),  color_img)
